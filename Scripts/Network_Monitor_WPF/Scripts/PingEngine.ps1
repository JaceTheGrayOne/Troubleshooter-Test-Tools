function Initialize-NMPingEngine {
    ...
    $script:NMPingSkippedTicks = 0
    $script:NMPingDisposedCount = 0

    $script:NMPingTimer = [System.Windows.Threading.DispatcherTimer]::new()
    $script:NMPingTimer.Interval = [TimeSpan]::FromMilliseconds([int]$script:NMConfig.RefreshMilliseconds)
    $script:NMPingTimer.Add_Tick({ Invoke-NMPingCycle })

    $script:NMPingCompletionTimer = [System.Windows.Threading.DispatcherTimer]::new()
    $script:NMPingCompletionTimer.Interval = [TimeSpan]::FromMilliseconds(50)
    $script:NMPingCompletionTimer.Add_Tick({ Complete-NMPingCycleIfReady })
}

function Test-NMMonitoringEnabled {
    return ($script:NMPingTimer -and $script:NMPingTimer.IsEnabled)
}
...
function Start-NMMonitoring {
    ...
    if (-not $script:NMPingTimer.IsEnabled) {
...
function Invoke-NMPingCycle {
    ...
    if ($script:NMPingCycleBusy) {
        $script:NMPingSkippedTicks++
        return
    }
